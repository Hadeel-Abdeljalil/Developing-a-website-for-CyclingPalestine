...
export default function Products() {
  ...
  const handleImageClick = (imageUrl, index) => {
    ...
    const updatedOpacity = { main: 0.5 };
    Object.keys(imageOpacity).forEach((key) => {
      updatedOpacity[key] = key === index.toString() ? 1 : 0.5;
    });
  ...
  if (isError) return <div>خطأ في جلب البيانات</div>;
  if (isLoading || !product) return <div className="loading bg-transfer w-100 d-flex justify-content-center align-items-center z-3"><span className="loader"></span></div>;
  ...
  return (
    <div className="container pt-5 mt-5 pb-5 mb-5">
              ...
              <div className="d-flex pe-2 dir">
                ...
                <div className="d-flex gap-1">
                  {Array.from({ length: avgRat() }, (_, index) => (
                    <svg key={index} height="800px" width="800px" viewBox="0 0 47.94 47.94" fill="#ED8A19">
                  ...
                  {Array.from({ length: 5 - avgRat() }, (_, index) => (
                    <svg key={index} height="200px" width="200px" viewBox="0 0 47.94 47.94" fill="#000000" stroke="#000000">
                      <path style={{ fill: '#ffffff' }} d="M26.285,2.486l5.407,10.956c0.376,0.762,1.103,1.29,1.944,1.412l12.091,1.757c2.118,0.308,2.963,2.91,1.431,4.403l-8.749,8.528c-0.608,0.593-0.886,1.448-0.742,2.285l2.065,12.042c0.362,2.109-1.852,3.717-3.746,2.722l-10.814-5.685c-0.752-0.395-1.651-0.395-2.403,0l-10.814,5.685c-1.894,0.996-4.108-0.613-3.746-2.722l2.065-12.042c0.144-0.837-0.134-1.692-0.742-2.285l-8.749-8.528c-1.532-1.494-0.687-4.096,1.431-4.403l12.091-1.757c0.841-0.122,1.568-0.65,1.944-1.412l5.407-10.956C22.602,0.567,25.338,0.567,26.285,2.486z" />
                    </svg>
                  ))}
                </div>
              </div>
              <div className="d-flex justify-content-center">
                <button className="btn text-white bg-dark d-flex align-items-center dir add-button" onClick={handleAddToCart} hidden={!userToken}>
                  <span className="ps-2 pb-1"><BiCart /></span>
                  أضف إلى عربة التسوق
                </button>
                <div className="heart-icon pe-3 pt-2"><BsHeart /></div>
              </div>
            </div>
            <div className="col-lg-6 d-flex">
              <div>
                <ReactImageMagnify
                  smallImage={{ alt: 'Wristwatch by Ted Baker London', isFluidWidth: false, src: selectedImage, width: 500, height: 600 }}
                  largeImage={{ src: selectedImage, width: 1200, height: 1800 }}
                  enlargedImageContainerDimensions={{ width: 300, height: 400 }}
                  isHintEnabled={true}
                ...
                {product.subImages && product.subImages.length > 0 ? (
                  <>
                    <div className="mt-2 ms-2">
                      <img src={product.mainImage.secure_url} alt="main image" className="sub-image" style={{ opacity: imageOpacity["main"] }} onClick={() => handleImageClick(product.mainImage.secure_url, "main")} />
                    </div>
                    {product.subImages.map((subImage, index) => (
                      <div key={index} className="mt-2 ms-2">
                        <img src={subImage.secure_url} alt={`sub image ${index}`} className="sub-image" style={{ opacity: imageOpacity[index] }} onClick={() => handleImageClick(subImage.secure_url, index)} />
                      </div>
                    ))}
                  </>
                ) : (
      ...
      <hr />
      <h2 className="text-center py-4 fw-bolder">ما مدى رضاك عن منتجنا؟</h2>
